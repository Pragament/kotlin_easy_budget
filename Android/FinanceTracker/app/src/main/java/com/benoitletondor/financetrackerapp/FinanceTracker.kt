...
import com.benoitletondor.FinanceTrackerapp.helper.*
import com.benoitletondor.FinanceTrackerapp.iab.Iab
import com.benoitletondor.FinanceTrackerapp.notif.*
import com.benoitletondor.FinanceTrackerapp.parameters.*
import com.benoitletondor.FinanceTrackerapp.push.PushService.Companion.DAILY_REMINDER_KEY
import com.benoitletondor.FinanceTrackerapp.push.PushService.Companion.MONTHLY_REMINDER_KEY
...
/**
 * FinanceTracker application. Implements GA tracking, Batch set-up, Crashlytics set-up && iab.
 *
...
// ------------------------------------------>
    ...
    private fun onUpdate(previousVersion: Int, @Suppress("SameParameterValue") newVersion: Int) {
        ...
        if (previousVersion < 132) {
            GlobalScope.launch {
                try {
                    if (Build.VERSION.SDK_INT >= 33 && ActivityCompat.checkSelfPermission(this@FinanceTracker, Manifest.permission.POST_NOTIFICATIONS) != PackageManager.PERMISSION_GRANTED) {
                        return@launch
                    }
                    ...
                    val intent = Intent(this@FinanceTracker, MainActivity::class.java).apply {
                        flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK
                    }
                    val pendingIntent: PendingIntent = PendingIntent.getActivity(this@FinanceTracker, 0, intent, PendingIntent.FLAG_IMMUTABLE)

                    val builder = NotificationCompat.Builder(this@FinanceTracker, CHANNEL_NEW_FEATURES)
                    ...
                    with(NotificationManagerCompat.from(this@FinanceTracker)) {
